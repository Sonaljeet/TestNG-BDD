...
import mars.JCI.Project.CSD.PerformanceCSD.CSD_Performance_CSDLogin_Page_Action;
import mars.JCI.Project.CSD.PerformanceCSD.CSD_Performance_GGL_ScreenShots;
import mars.JCI.Project.CSD.PerformanceCSD.CSD_Performance_HandleJTLFile;

/**
 * @author cdeyso
 *
 */
public class JMeterAPISampleTestCSD extends BaseClass {
	
	public static int loopCount = 1;
	public static int userThreads = 1;
	public static int jsonVal = 1;
	

	@SuppressWarnings("static-access")
	@Test(priority=0,description="Running a POC on Performace Test In MARS Framework using JMeter Libraries! -- Login to CSD Application")
	public static void RunPerformanceTestCSDLogin() {
		
		try {
			
			CSD_Performance_CSDLogin_Page_Action performance_PA = new CSD_Performance_CSDLogin_Page_Action(logger);
			CSD_Performance_HandleJTLFile performance_JTL = new CSD_Performance_HandleJTLFile(logger);
			performance_PA.ExecuteJMeter(loopCount,userThreads,jsonVal);
