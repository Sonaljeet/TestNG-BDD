...
public class DLS_Option_Name_Search_Page_Action {
	/** The Home page title. */
	...
	public DLS_Option_Name_Search_Page_Action(MARSDesktopDriver desktopDriver, ExtentTest logger) {
		this.desktopDriver = desktopDriver;
		this.logger = logger;

	}

	public void selectOptionName(String optionName) {
		desktopDriver.marsSleep(3000);
		desktopDriver.marsMouseClick(text, 461, 128, 1, 0);
		desktopDriver.marsSend(optionName, 1);
		logger.log(LogStatus.PASS, "Option Name is Entered Successfully to Option Name Search WindowElement");
		desktopDriver.marsMouseClick(text, 519, 138, 1, 0);
		logger.log(LogStatus.PASS, "Option Name Search done Successfully");
