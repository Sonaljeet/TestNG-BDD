...
public class CCT_Sideloop_Page_Action {
	...
	/** The Side loop Page Title */
	public String sideloopPageTitle = "Sideloop";

	public CCT_Sideloop_Page_Action(MARSDesktopDriver desktopDriver, ExtentTest logger, String baseurl) {
		this.desktopDriver = desktopDriver;
		this.logger = logger;
		this.BaseURL = baseurl;
	}

	public void cctSideloop() throws InterruptedException, IOException {
		...
		if (desktopDriver.marsWinWaitActive(homePageTitle, text, 20) > 0) {
			desktopDriver.marsMouseClick("", 1549, 69, 1, 0);
			Thread.sleep(8000);
			
				desktopDriver.marsMouseClick("", 1059, 600, 21, 0);
				Thread.sleep(4000);
				desktopDriver.marsMouseClick("", 584, 402, 1, 0);
				Thread.sleep(1000);
				desktopDriver.marsMouseClick("", 838, 627, 1, 0);
				Thread.sleep(8000);

				desktopDriver.marsMouseClick("", 800, 421, 1, 0);
				desktopDriver.marsControlSend(sideloopPageTitle, text, "", "ABC");
				Thread.sleep(1000);
				desktopDriver.marsMouseClick("", 761, 455, 1, 0);

				JavaTimer.startSessionTimer();

				if(desktopDriver.marsWinWaitNotActive(selectionPageTitle, text, 20) > 0){
					if (desktopDriver.marsWinWait(homePageTitle, text, 20) > 0) {

						logger.log(LogStatus.PASS, "Sideloop Creation took  " + JavaTimer.endSessionTimer());
					}
