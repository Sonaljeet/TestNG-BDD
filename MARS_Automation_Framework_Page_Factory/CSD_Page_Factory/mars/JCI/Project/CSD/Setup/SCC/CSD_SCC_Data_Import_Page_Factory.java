...
public class CSD_SCC_Data_Import_Page_Factory {
	...
	//The SCC Equipment Type Selection RadioButton -- Existing
	@FindBy(css="input[name=sccType][value=isExisting]")
	private WebElement sccTab_sccExisitngEquipmentTypeLink;
		
	public WebElement get_sccTab_sccExisitngEquipmentTypeLink(){
		if(commonFunctions.WebElementCommon.waitForElementPresent(driver, sccTab_sccExisitngEquipmentTypeLink, logger)){
			return sccTab_sccExisitngEquipmentTypeLink;
		}else
			return null;
	}
	
	//The SCC Equipment Type Selection RadioButton -- New
	@FindBy(css="input[name='sccType'][value='isNew']")
	private WebElement sccTab_sccNewEquipmentTypeLink;
		
	public WebElement get_sccTab_sccNewEquipmentTypeLink(){
		if(commonFunctions.WebElementCommon.waitForElementPresent(driver, sccTab_sccNewEquipmentTypeLink, logger)){
			return sccTab_sccNewEquipmentTypeLink;
		}else
			return null;
	}
	
	//The SCC Equipment Chiller Name Entry TextBox -- New
	@FindBy(css="input[name='txtChiller']")
	private WebElement sccTab_sccNewEquipmentNameLink;
		
	public WebElement get_sccTab_sccNewEquipmentNameLink(){
		if(commonFunctions.WebElementCommon.waitForElementPresent(driver, sccTab_sccNewEquipmentNameLink, logger)){
			return sccTab_sccNewEquipmentNameLink;
		}else
			return null;
	}
	...
	//The TextBox to enter the FQR Point Status for checking the validity of the point
	@FindBy(css="input[type=text][name=status]")
	private WebElement sccTab_inputFQRPointStatusLink;
		
	public WebElement get_sccTab_inputFQRPointStatusLink(){
		if(commonFunctions.WebElementCommon.waitForElementPresent(driver, sccTab_inputFQRPointStatusLink, logger)){
			return sccTab_inputFQRPointStatusLink;
		}else
			return null;
	}
	
	//The Single Table Entry after a successful Point is entered into the FQR Set  --- STATUS Entry for the Points for the selected Chiller
	@FindBy(css="#grdAssetDetail > table > tbody > tr > td:nth-child(4)")
	private WebElement sccTab_outputFQRPointStatusLink;
		
	public WebElement get_sccTab_outputFQRPointStatusLink(){
		if(commonFunctions.WebElementCommon.waitForElementPresent(driver, sccTab_outputFQRPointStatusLink, logger)){
			return sccTab_outputFQRPointStatusLink;
		}else
			return null;
	}
	
	//The Entry to check after the points are imported
	@FindBy(css="table[ng-table='tblAssetDetails']>tbody>tr")
	private List<WebElement> sccTab_outputFQRPointImportResultLink;
		
	public List<WebElement> get_sccTab_outputFQRPointImportResultLink(){
		if(commonFunctions.WebElementCommon.waitForElementsPresent(driver, sccTab_outputFQRPointImportResultLink, logger)){
			return sccTab_outputFQRPointImportResultLink;
		}else
			return null;
	}
}
